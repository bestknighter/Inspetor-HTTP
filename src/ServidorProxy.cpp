...
bool ServidorProxy::Loop() {
	...
	for( int i = il.messagesReceived.size(); i++ ) {
		...
		el.Send( il.messagesReceived[i].connectionID, il.messagesReceived[i].addr_to, il.messagesReceived[i].port_to, il.messagesReceived[i].message );
	}

	// Arriving machine last
	el.ReceiveMessages();
	printf( "\nThere are %d inbound packets.\n", el.messagesReceived.size() );
	for( int i = el.messagesReceived.size(); i++ ) {
		printf( "\nMessage from %s:%d to %s:%d\n%s\n"
			, il.messagesReceived[i].addr_from.c_str()
			, il.messagesReceived[i].port_from
			, il.messagesReceived[i].addr_to.c_str()
		...
		il.Send( el.messagesReceived[i].connectionID, el.messagesReceived[i].message );
	}
}
